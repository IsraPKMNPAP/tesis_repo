...
from src.models.mm_vae_audio_crossatt import CrossAttentiveMMVAEAudio
...
def main():
    ...
    ap.add_argument("--shared-dim", type=int, default=64, help="Latent dim (mu/logvar)")
    ap.add_argument("--fusion-dim", type=int, default=128, help="Dimension de tokens de fusion/atencion")
    ...
    ap.add_argument("--w-rec-aud", type=float, default=1.0)
    ap.add_argument("--w-cls", type=float, default=1.0)
    ap.add_argument("--w-kl", type=float, default=1.0)
    ap.add_argument("--kl-anneal-start", type=float, default=0.0)
    ap.add_argument("--kl-anneal-end", type=float, default=1.0)
    ...
    ap.add_argument("--w-gate-reg", type=float, default=0.0, help="Regularizacion de gates (entropia baja)")
    ...
    ap.add_argument("--fuse-dropout-at-cls", type=float, default=0.0, help="Alias antiguo (compatibilidad)", dest="fuse_dropout_alias")
    ...
    ap.add_argument("--w-aux-aud", type=float, default=0.1, help="Peso de CE auxiliar sobre logits_aud")
    # Modality toggles
    ...
    pre_hash = compute_run_hash(tmp_cfg, sys.argv, model="CrossAtt_Preproc")
    save_model_pickle(preprocessor, results_dir / artifact_name("CrossAttVAEAudio", "preprocessor", pre_hash, "pkl"))
    ...
    model = CrossAttentiveMMVAEAudio(
        ...
        fuse_dropout=args.fuse_dropout if args.fuse_dropout is not None else args.fuse_dropout_alias,
    )
    ...
    for epoch in range(args.epochs):
        ...
        tr_con = tr_cls = tr_rec_tab = tr_rec_vid = tr_rec_aud = tr_kl = tr_aux_tab = tr_aux_vid = tr_aux_aud = tr_gate = 0.0
        tr_batches = 0
        ...
        for b in train_iter:
            ...
            w_rec_aud = args.w_rec_aud if (use_aud and x_aud is not None) else 0.0
            ...
            tr_rec_aud += float(logs.get("rec_aud", 0.0))
            ...
            tr_gate += float(logs.get("gate", 0.0))
            tr_batches += 1
            global_step += 1
        ...
        avg_rec_aud = (tr_rec_aud / max(1, tr_batches))
        ...
        avg_gate = (tr_gate / max(1, tr_batches))
        ...
        print(
            f"Epoch {epoch+1}/{args.epochs} | train_loss={history['loss'][-1]:.4f} | train_acc={tr_acc:.3f} | val_acc={val_acc:.3f} | con={avg_con:.3f} | rec_aud={avg_rec_aud:.3f} | gate={avg_gate:.3f} | lr={cur_lr:.2e}"
        )
    ...
    cfg = {
        ...
        "video_trainable": args.video_trainable,
        "video_unfreeze_last": args.video_unfreeze_last,
        "video_target_size": args.video_target_size,
        "video_lstm_hidden": args.video_lstm_hidden,
        "video_lstm_layers": args.video_lstm_layers,
        "video_bidirectional": args.video_bidirectional,
