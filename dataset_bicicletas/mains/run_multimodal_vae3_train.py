...
from src.models.mm_vae3 import DeterministicMMVAE3, VariationalMMVAE3
...
def main():
    ...
    ap.add_argument("--freeze-video", action="store_true", default=True, help="Congela backbone de video")
    ap.add_argument("--freeze-audio", action="store_true", default=True, help="Congela encoder de audio")
    ap.add_argument("--unfreeze-video", action="store_true", help="Descongela backbone de video")
    ap.add_argument("--unfreeze-audio", action="store_true", help="Descongela encoder de audio")
    ...
    ap.add_argument("--rec-emb-dim", type=int, default=128)
    ap.add_argument("--dropout", type=float, default=0.0)
    ap.add_argument("--w-rec-tab", type=float, default=0.05)
    ap.add_argument("--w-rec-vid", type=float, default=0.2)
    ap.add_argument("--w-rec-aud", type=float, default=0.2)
    ...
    ap.add_argument("--audio-encoder", type=str, default="wav2vec", choices=["simple", "cnn", "tcn", "wav2vec"])
    ...
    args = ap.parse_args()
    if args.unfreeze_video:
        args.freeze_video = False
    if args.unfreeze_audio:
        args.freeze_audio = False
    ...
    pre_hash = compute_run_hash(tmp_cfg, sys.argv, model="MMVAE3_Preproc")
    save_model_pickle(preprocessor, results_dir / artifact_name("MMVAE3", "preprocessor", pre_hash, "pkl"))
    ...
    if args.debug_aux_step:
        ...
        if isinstance(model, VariationalMMVAE3):
    ...
    for epoch in range(args.epochs):
        ...
        tr_align = tr_con = tr_cls = tr_rec_tab = tr_rec_vid = tr_rec_aud = tr_kl = tr_aux_tab = tr_aux_vid = tr_aux_aud = 0.0
        tr_batches = 0
        ...
        for b in train_iter:
            ...
            if epoch < int(args.warmup_epochs):
                ...
                w_rec_aud = args.w_rec_aud if use_aud else 0.0
        ...
        if dl_val is not None:
            ...
            with torch.no_grad():
                for b in dl_val:
                    ...
                    if epoch < int(args.warmup_epochs):
                        use_tab = False
                        if not (use_vid_default or use_aud_default):
                            use_tab = True
                    else:
                        use_tab = use_tab_default
                    if not use_tab:
        ...
        avg_rec_aud = float(np.nan_to_num(tr_rec_aud / max(1, tr_batches)))
        ...
        print(
            ...
            f"rec_tab={avg_rec_tab:.3f} | rec_vid={avg_rec_vid:.3f} | rec_aud={avg_rec_aud:.3f} | "
    ...
    # Results
    model_name = "MMVAE3_Det" if args.deterministic else "MMVAE3_Var"
    cfg = {
        "pkl": str(pkl_path),
        "label_col": args.label_col,
        "path_col": args.path_col,
    ...
    save_metrics(all_metrics, results_dir, model_name, cfg, run_hash)
