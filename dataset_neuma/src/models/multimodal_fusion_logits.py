...
class FusionClassifierLogits(nn.Module):
    """
    Variante determinista donde la última capa del head es directamente el vector de logits
    (dimension = num_options), sin proyección adicional ni softmax.
    """

    def __init__(self, tab_dim: int, clip_dim: int, eeg_ch: int, tab_out: int = 64, eeg_out: int = 64, hidden=(128, 64), dropout: float = 0.2, img_proj: int = 0, logit_dim: int = 1):
        super().__init__()
        ...
        for h in hidden:
            layers.extend([nn.Linear(last, h), nn.ReLU(), nn.Dropout(dropout)])
            last = h
        # Última capa oculta ya es el vector de logits (sin softmax)
        layers.append(nn.Linear(last, logit_dim))
...
class FusionVAELogits(nn.Module):
    """
    Variante variacional donde el decodificador termina en un vector de logits de tamaño logit_dim,
    sin capa extra ni softmax.
    """

    def __init__(self, tab_dim: int, clip_dim: int, eeg_ch: int, tab_out: int = 64, eeg_out: int = 64, hidden=(256, 128), latent_dim: int = 64, dropout: float = 0.2, img_proj: int = 0, logit_dim: int = 1):
        super().__init__()
        self.tab_enc = TabEncoder(tab_dim, hidden=(128,), out_dim=tab_out, dropout=dropout)
        self.eeg_enc = EEGEncoder(eeg_ch, hidden=32, out_dim=eeg_out, dropout=dropout) if eeg_ch > 0 else None
        self.img_proj = None
    ...
    def forward(self, tab: torch.Tensor, clip: torch.Tensor, eeg: torch.Tensor) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        ...
        logits = self.dec(z)
        return logits.squeeze(-1) if logits.shape[-1] == 1 else logits, mu, logvar
